...
router.delete("/:id", authenticate, async (req, res) => {
  try {
    if (req.user.rol !== "admin") {
      return res.status(403).json({ msg: "Solo admin puede eliminar anomalías" });
    }
    const result = await pool.query("DELETE FROM anomalias WHERE id = $1 RETURNING id", [req.params.id]);
    if (result.rows.length === 0) return res.status(404).json({ msg: "Anomalía no encontrada" });
    res.json({ msg: "Anomalía eliminada" });
  } catch (err) {
    console.error(err.message);
    res.status(500).json({ msg: "Error del servidor" });
  }
